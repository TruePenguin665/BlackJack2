...
public class CardDealer : MonoBehaviour
{
    ...
    private void PlaceBet(int amount)
    {
        if (canPlaceBet && bankroll >= amount)
        {
            ...
            // Add initial bet for the hand if it's not already added
            if (handBets.Count == 0)
            {
                handBets.Add(currentBet);  // Track the bet for the first hand
            }
        }
    ...
    private void StartNewRound()
    {
        ...
        {
            // Dealer has blackjack, handle result
            HandleDealerBlackjack();
        }
        else
        {
            // Normal round continues if no dealer blackjack
            UnityEngine.Debug.Log("No dealer blackjack. Continue with player actions.");
            roundActive = true;
    ...
    private void CheckWinConditions()
    {
        ...
        ResetBet();  // Reset bet for the next round
        EndRound("Round over.");
    }
